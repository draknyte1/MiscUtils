...
public class MachineUtil
{
  ...
  public boolean hasIngredients(int[] recipe, int[] inventory)
  {
    ...
    ItemStack[] arrayOfItemStack1 = getStacks(recipe);int i = arrayOfItemStack1.length;
    for (ItemStack localItemStack1 = 0; localItemStack1 < i; localItemStack1++)
    {
      stack = arrayOfItemStack1[localItemStack1];
      requiredStacks.add(stack);
    }
    ItemStackSet inventoryStacks = new ItemStackSet();
    ItemStack[] arrayOfItemStack2 = getStacks(inventory);localItemStack1 = arrayOfItemStack2.length;
    for (ItemStack stack = 0; stack < localItemStack1; stack++)
    {
      ItemStack stack2 = arrayOfItemStack2[stack];
