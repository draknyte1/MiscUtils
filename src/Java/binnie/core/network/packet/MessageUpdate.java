  ...
  extends MessageCoordinates
{
    ...
    throws IOException
  {
    ...
    for (Iterator localIterator = this.payload.intPayload.iterator(); localIterator.hasNext();)
    {
      int intData = ((Integer)localIterator.next()).intValue();
      data.writeInt(intData);
    }
    for (localIterator = this.payload.floatPayload.iterator(); localIterator.hasNext();)
    {
      float floatData = ((Float)localIterator.next()).floatValue();
