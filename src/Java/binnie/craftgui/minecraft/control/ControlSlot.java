  ...
  public ControlSlot(IWidget parent, float x, float y)
  {
    ...
    addSelfEventHandler(new EventMouse.Down.Handler()
    {
      public void onEvent(EventMouse.Down event)
      {
        ...
        {
          PlayerControllerMP playerController = ((Window)ControlSlot.this.getSuperParent()).getGui().getMinecraft().playerController;
          int windowId = ((Window)ControlSlot.this.getSuperParent()).getContainer().windowId;
          int slotNumber = ControlSlot.this.slot.slotNumber;
  ...
  public void onRenderBackground()
  {
    ...
    for (Map.Entry<EnumHighlighting, List<Integer>> highlight : highlighting.entrySet()) {
      if (((highlight.getKey() != EnumHighlighting.ShiftClick) || (shiftClickActive)) && 
        ...
        (!highlighted) && (((List)highlight.getValue()).contains(Integer.valueOf(this.slot.slotNumber))))
      {
        highlighted = true;
