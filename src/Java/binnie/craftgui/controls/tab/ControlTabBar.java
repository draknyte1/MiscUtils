  ...
  implements IControlValue<T>
{
  ...
  public void setValues(Collection<T> values)
  {
    ...
    for (T value : values)
    {
      ControlTab localControlTab;
      if ((this.position == Position.Top) || (this.position == Position.Bottom)) {
        localControlTab = createTab(j * tabDimension, 0.0F, tabDimension, getSize().y(), value);
      } else {
        localControlTab = createTab(0.0F, j * tabDimension, getSize().x(), tabDimension, value);
      }
      j++;
    }
