  ...
  extends ControlProgressBase
{
  ...
  public void onRenderBackground()
  {
    ...
    float range = w();
    float h = 8.0F;
    float ooy = -((n - 1.0F) * 10.0F) - range / 2.0F;
    float ddy = (n - 1.0F) * 10.0F + range;
    ...
    for (int i = 0; i < n; i++)
    {
      ...
      if ((y > -range / 2.0F) && (y < range / 2.0F))
      {
        ...
        float offMovement = (h() - 16.0F) / 2.0F;
        int alpha = 16777216 * (int)(255.0F * percentView);
        c1 += alpha;
        c2 += alpha;
        c3 += alpha;
        c4 += alpha;
        CraftGUI.Render.solidAlpha(new IArea(w() / 2.0F + y, offMovement * percentView, 4.0F, 4.0F), c1);
        CraftGUI.Render.solidAlpha(new IArea(w() / 2.0F + y, offMovement * percentView + 4.0F, 4.0F, 4.0F), y < 0.0F ? c2 : c3);
        CraftGUI.Render.solidAlpha(new IArea(w() / 2.0F + y, h() - offMovement * percentView - 8.0F, 4.0F, 4.0F), y < 0.0F ? c3 : c2);
        CraftGUI.Render.solidAlpha(new IArea(w() / 2.0F + y, h() - offMovement * percentView - 4.0F, 4.0F, 4.0F), c4);
      }
    }
  }
}
